...
def parse_contents(contents, filename):
    ...
    try:
        if "csv" in filename:
            ...
            return html.Div(["Archivo no soportado: {}".format(filename)])
    except Exception as e:
        print(e)
        return html.Div(["Hubo un error al procesar este archivo."])
...
def create_map_figure(df, polygon_geojson=None):
    ...
    if polygon_geojson is not None:
        gdf = gpd.GeoDataFrame.from_features(
            polygon_geojson["features"]
        )  # crea un gejson a partir del dicionario features
        ...
        choropleth_layer = go.Choroplethmapbox(
            ...
            locations=gdf.index.astype(
                str
            ),  # enumera todos los poligonos y is indices los vuelve texto
...
def perform_kde(df, contour_levels=12):
    ...
    x_grid, y_grid = np.meshgrid(
        np.linspace(min(x), max(x), 400), np.linspace(min(y), max(y), 200)
    )
    ...
    contours = plt.contour(
        x_grid, y_grid, influence, levels=contour_levels, colors="k"
    ).allsegs
...
def generate_map(df_dict, current_figure, current_config, kde_output):
    ...
    triggered_by_kde_output = (
        ctx.triggered and ctx.triggered[0]["prop_id"] == "kde-output.data"
    )
    ...
    if (
        ...
        and not triggered_by_kde_output
    ):
        ...
        if scatter_trace is not None:
            ...
            scatter_trace.customdata = filtered_df[
                ["Clase"]
            ].values  # Actualizar las clases
...
def toggle_offcanvas(n1, n2, df_dict, is_open, input_values, store_data):
    ...
    if not ctx.triggered:
        ...
        if "open-offcanvas" in prop_id:
            ...
            if not store_data:
                store_data = transform_df(
                    df_dict
                )  # carga los valores a los inputs pero no abre la ventana
            return (
                not is_open,
                [store_data.get(str(i), "") for i in range(14)],
                store_data,
            )
...
def update_dataframe(*args):
    ...
    df.loc[df.value_c.isin(["CCOM", "MERC", "REST", "UNIV"]), ["Peso", "Radio"]] = (
        pesos[6],
        radios[6],
    )
...
def export_dataframe(n_clicks, store_data, intermediate_data):
    if n_clicks is None:  # el botón no ha sido presionado
        ...
        df.loc[
            (df["Clase"] == "Agencia")
            & (df["Banco"] == "Banco Nacional de Bolivia S.A."),
            "Peso",
        ] = new_weights[0]
        df.loc[
            (df["Clase"] == "Agencia")
        ...
        df.loc[
            df["Clase"].isin(
                ["Centro Comercial", "Mercado", "Restaurante", "Universidad"]
            ),
            "Peso",
        ] = new_weights[6]
        df.loc[
